...
## 安全与合规
- 请勿将包含密钥、密码、访问令牌的文件提交到仓库（`.env` 已在 `.gitignore` 中）
- 邮件等外部工具的真实调用需遵循公司安全策略与合规要求
- 如需对外网服务，确保网络与代理配置合规


## 常见问题（FAQ）
1) 启动报错找不到依赖？请确认已在虚拟环境中安装依赖，并使用正确的 Python 版本。
2) 嵌入生成失败或超时？请检查 `.env` 中的模型与 BASE_URL、网络连通性与 API Key 可用性。
3) 前端打不开或空白？请确认后端 API 正在运行，且前端的 `API_BASE_URL` 配置为 `http://localhost:8000/api/v1`（见 `src/frontend/app.py`）。
4) 向量库无结果？请先在“文档管理”页上传文档或通过 API 将文档入库。
